...
ADDR_USDC=0x879bad9DcD7e7f79B598a632103984FC090DA00D

NETWORK=mumbai_special
...
LOGS_PARAMS_FILE=logs/addresses.mumbai_special.log
...
echo "ETH_TEST_PRIVATE_KEYS = [\"0x$PRIVATE_KEY\"]" >> ./.env
...
echo "USDC $ADDR_USDC" >> $LOGS_PARAMS_FILE;
...
CUR_COMMAND="";
if [ $EXEC_TYPE == $EXEC_TYPE_NORMAL ]; then
    CUR_COMMAND="npx truffle exec scripts/bns/2_configure_contracts.js --market $ADDR_DOMAIN_MARKET --nft $ADDR_DOMAIN_NFT --router $ADDR_DOMAIN_ROUTER --provider $ADDR_DOMAIN_PROVIDER --provider1 $ADDR_CONTENT_PROVIDER --dividends $ADDR_INVESTING_DIVIDENDS --usdt $ADDR_USDT --busd $ADDR_BUSD --usdc $ADDR_USDC --network $NETWORK";
else
...
CUR_COMMAND="";
if [ $EXEC_TYPE == $EXEC_TYPE_NORMAL ]; then
    CUR_COMMAND="npx truffle exec scripts/investing/2_configure_contracts.js --market $ADDR_INVESTING_MARKET --pricing $ADDR_INVESTING_PRICING --nft $ADDR_INVESTING_NFT --dividends $ADDR_INVESTING_DIVIDENDS --usdt $ADDR_USDT --busd $ADDR_BUSD --busc $ADDR_USDC --devs $ADDR_DEVS --team $ADDR_TEAM --fund $ADDR_FUND --network $NETWORK";
else
